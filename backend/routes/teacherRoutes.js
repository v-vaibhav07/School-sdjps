...
//   const { full_name, email, subject, phone, qualification, experience } = req.body
...
router.get("/", async (req, res) => {
  try {
    ...
    console.log("GET TEACHERS SERVER ERROR:", err)
    res.status(500).json({ error: "Server error" })
  }
})
  ...
  try {
    ...
    if (error) {
      if (
        ...
        return res.json({
          message: "Teacher updated successfully ✏️",
          teacher: updated
        })
      }
...
router.put("/:id", async (req, res) => {
  ...
  try {
    ...
    if (userUpdateError) {
      console.log("UPDATE USER ERROR:", userUpdateError)
    ...
    if (teacherUpdateError) {
      console.log("UPDATE TEACHER ERROR:", teacherUpdateError)
      return res.status(400).json({ error: teacherUpdateError.message })
    }
    ...
    console.log("UPDATE TEACHER SERVER ERROR:", err)
    res.status(500).json({ error: "Server error" })
  }
})
...
router.post("/pay", async (req, res) => {
  ...
  try {
    ...
    if (existingError) {
      console.log("CHECK PAYMENT ERROR:", existingError)
    ...
    if (error) {
      console.log("PAY SALARY ERROR:", error)
      return res.status(400).json({ error: error.message })
    }

    res.json({ message: "Salary Paid Successfully ✅" })
  } catch (err) {
    console.log("PAY SALARY SERVER ERROR:", err)
    res.status(500).json({ error: "Server error" })
  }
})
...
router.get("/my-salary", verifyToken, async (req, res) => {
  ...
  try {
    ...
    if (teacherError) {
      console.log("MY SALARY TEACHER ERROR:", teacherError)
    ...
    if (error) {
      console.log("MY SALARY PAYMENTS ERROR:", error)
      return res.status(400).json({ error: error.message })
    }

    res.json(data)
  } catch (err) {
    console.log("MY SALARY SERVER ERROR:", err)
    res.status(500).json({ error: "Server error" })
  }
})
...
router.get("/:id/payments", async (req, res) => {
  ...
  try {
    ...
    console.log("PAYMENT HISTORY SERVER ERROR:", err)
    res.status(500).json({ error: "Server error" })
  }
})
...
router.delete("/:id", async (req, res) => {
  ...
  try {
    ...
    if (teacherDeleteError) {
      console.log("DELETE TEACHER ROW ERROR:", teacherDeleteError)
      return res.status(400).json({ error: teacherDeleteError.message })
    }
    ...
    if (userDeleteError) {
      console.log("DELETE USER ERROR:", userDeleteError)
    ...
    console.log("DELETE TEACHER SERVER ERROR:", err)
    res.status(500).json({ error: "Server error" })
  }
})
